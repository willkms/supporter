...
def add_predict_grade(grade_list):
    ...
    for i in range(len(grade_list)):
        ...
        student_grade_df = student_grade_df.astype({"student_id": "int16", "test_id": "int16"})        
        ...
        # predict_df = predict_df.astype({"test_id": "int16"})
        ...
        if not math.isnan(float(test_id)):
            ...
            if grade_list[i][-1][1] != "-1":
                ...
                for j in range(test_id, 7):
                    ...
                    # 空を含む生徒IDのグループを抽出
                    predict_df = predict_df.astype("int16")
                    ...
                    predict_df_score = predict_df_score.astype("int16")
                    predict_df_s_score = predict_df_s_score.astype("int16")
